...
class TestParse(TestCase):
    ...
    def test_event_counts(self):
        ...
        for fn, count in zip(self.files, counts):
            with open("tests/edls/" + fn, "r") as f:
                edl = pycmx.parse_cmx3600(f)
                actual = len(list(edl.events))
                self.assertTrue(
                    actual == count,
                    f"expected {count} in file {fn} but found {actual}"
                )
        ...
        for fn in self.files:
            ...
            with open(path, "r") as f:
                ...
                for index, event in enumerate(edl.events):
                    self.assertTrue(len(event.edits) > 0, f"Failed for {path}")
                    self.assertEqual(event.number, index + 1, f"Failed for {path}")
    ...
    def test_multi_edit_events(self):
        with open("tests/edls/TEST.edl", "r") as f:
            ...
            self.assertEqual(
                events[42].edits[0].clip_name, "TC R1 V1.2 TEMP1 FX ST.WAV"
            )
            ...
            self.assertTrue(
                events[42].edits[1].transition.kind == pycmx.Transition.Dissolve
            )
        ...
        with open("tests/edls/ISSUE_14_conform_edl_issue_03.edl", "r") as f:
            ...
            for event in edl.events:
                if event.number == 42:
                    ...
                    self.assertEqual(event.edits[0].source, "M018C0005_240925_1F4L13")
                    self.assertEqual(
                        event.edits[0].transition.kind, pycmx.Transition.Cut
                    )
                    self.assertEqual(event.edits[0].source_in, "18:44:20:12")
