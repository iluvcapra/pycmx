...
def parse_cmx3600_statements(file: TextIO, tolerant: bool = False) -> list[object]:
    ...
    return [
        _parse_cmx3600_line(line.strip(), line_number, tolerant)
...
def _parse_remark(line, line_number) -> object:
    if line.startswith("FROM CLIP NAME:"):
        return StmtClipName(
            name=line[15:].strip(), affect="from", line_number=line_number
        )
    elif line.startswith("TO CLIP NAME:"):
        return StmtClipName(
            name=line[13:].strip(), affect="to", line_number=line_number
        )
        ...
        if len(value) != 1:
            ...
            try:
                return StmtCdlSat(value=float(value[0][0]), line_number=line_number)
...
def _parse_split(line: str, line_number):
    ...
    return StmtSplitEdit(video=is_video, delay=split_delay, line_number=line_number)
...
def _parse_columns_for_standard_form(
    line: str, event_field_length: int, source_field_length: int, line_number: int
):
    # breakpoint()
